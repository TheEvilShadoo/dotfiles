...
local function initializeIfNeeded()
    ...
    mp.add_forced_key_binding("shift+q", "toggle-strip-metadata", function()
    ...
    mp.add_forced_key_binding("shift+r", "seek-to-start-position", function()
        mp.commandv("seek", startPosition, "absolute")
        mp.command("show-progress")
        updateTrimmingPositionsOSDASS()
    end)
    mp.add_forced_key_binding("shift+t", "seek-to-end-position", function()
...
-- Static Key Bindings
--
mp.add_key_binding("r", "trim-set-start-position", setStartPosition)
mp.add_key_binding("t", "trim-set-end-position", setEndPosition)
